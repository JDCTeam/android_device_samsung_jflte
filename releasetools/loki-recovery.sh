...
if [ $? -eq 0 ]; then
  ...
  /tmp/install/bin/loki_tool unlok $C/recovery.lok $C/recovery
else
...
if ! applypatch -c EMMC:$C/recovery:$RECSIZE:$RECSHA1; then
  if [ $need_lok -eq 1 ]; then
    ...
    /tmp/install/bin/loki_tool unlok $C/boot.lok $C/boot
  fi
  ...
  if [ $need_lok -eq 1 ]; then
    /tmp/install/bin/loki_tool patch recovery $C/aboot.img $C/recovery $C/recovery.lok || exit 1
    /tmp/install/bin/loki_tool flash recovery $C/recovery.lok || exit 1
  fi
